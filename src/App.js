...
function LoginScreen({ onLogin, error }) {
  return (
    ...
    >
      ...
      <div
        style={{
          background: 'linear-gradient(145deg,#0D1E33,#091525)',
    ...
    >
      ...
      >
        ...
        >
          ...
          , tu solicitud de acceso fue registrada. El administrador del despacho
          debe autorizarte para que puedas ver tu panel.
        </div>
        <div
          style={{
        ...
        >
          Contacta al administrador y comparte este correo:
          <br />
...
function AppCore({ user, role, linkedEmpId, onLogout }) {
    ...
    async (silent = false) => {
      ...
      try {
        if (isAdmin) {
          ...
          if (msgsData) {
            setMessages({
              [linkedEmpId]: Object.values(msgsData).sort(
                (a, b) => a.ts - b.ts
              ),
            });
          } else {
  ...
  /* ─── STATS ─── */
  const visibleEmps = isAdmin ? emps : emps.filter((e) => e.id === linkedEmpId);
  ...
  if (isAdmin) {
    Object.entries(messages).forEach(([eid, list]) => {
      unreadByEmp[eid] = list.filter(
        (m) => m.from === 'colaborador' && !m.readByAdmin
      ).length;
    });
  }
  const myUnread =
    role === 'colaborador' && linkedEmpId && messages[linkedEmpId]
      ? messages[linkedEmpId].filter((m) => m.from === 'admin' && !m.readByMe)
          .length
      : 0;
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#060F1A' }}>
      ...
      <header
        style={{
          background: 'linear-gradient(180deg,#0A1628,rgba(6,15,26,0.96))',
      ...
      <main style={{ padding: '44px 36px', maxWidth: 1440, margin: '0 auto' }}>
        ...
        {view === 'main' && isAdmin && (
          <div className="fu">
            ...
            >
              <div className="sec" style={{ color: '#42A5F5' }}>
                ◈ Colaboradores del Despacho
              </div>
              <button className="btn bp" onClick={() => setModal('addEmp')}>
                + Agregar Colaborador
              </button>
            </div>
            {emps.length === 0 && (
        ...
        {view === 'employee' &&
          selEmp &&
          (() => {
            const empCos = (cos[selEmp.id] || []).map((c) => ({
              ...c,
              ...coStats(c),
            }));
            const tot = empCos.reduce(
              (a, c) => ({
                done: a.done + c.done,
                ...
                >
                  <div
                    style={{
                      ...
                      height: 62,
                      borderRadius: 16,
                      background: `linear-gradient(135deg,${selEmp.color},${selEmp.color}bb)`,
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      fontSize: 22,
                      fontWeight: 700,
                      color: '#fff',
                      boxShadow: `0 10px 30px ${selEmp.color}44`,
                    }}
                  >
                    {selEmp.initials}
                  </div>
        ...
        {view === 'company' &&
          ...
          (() => {
            ...
            const pendingList = list.filter(
              (o) => !o.done && !isOverdue(o.due, o.done)
            );
            ...
            return (
              <div className="fu">
                ...
                {overdues.map((o) => (
                  <div key={o.id} className="alr">
                    ...
                    <div>
                      ...
                      <span
                        style={{ color: 'rgba(239,83,80,0.75)', fontSize: 13 }}
                      >
                        {' '}
                        — venció hace {daysLate(o.due)} día(s) · Fecha: {o.due}
                      </span>
                    </div>
                  </div>
                ))}
                ...
                >
                  ...
                  <button className="btn bp" onClick={() => setModal('addOb')}>
                    + Nueva Obligación
                  </button>
                </div>
                {list.length === 0 && (
                ...
                {secs.map(({ label, items, cls, color, bg }) => (
                  <div key={label} style={{ marginBottom: 34 }}>
                    ...
                    {items.map((ob) => (
                      ...
                      >
                        <button
                          className={`ck ${ob.done ? 'done' : ''}`}
                          onClick={async () => {
                            const upd = list.map((o) =>
                              o.id === ob.id
                                ? {
                                    ...o,
                                    done: !o.done,
                                    doneDate: !o.done ? todayISO() : null,
                                  }
                                : o
                            );
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div
                            style={{
                              fontSize: 15,
                              fontWeight: 600,
                              color: ob.done ? '#66BB6A' : '#E2D9C8',
                              textDecoration: ob.done ? 'line-through' : 'none',
                              marginBottom: 5,
                            }}
                          >
                            {ob.name}
                            ...
                            <button
                              ...
                              onClick={async () => {
                                const n = {
                                  ...ob,
                                  id: uid(),
                                  due: addMonths(ob.due, FREQ_M[ob.freq]),
                                  done: false,
                                  doneDate: null,
                                };
      ...
      {role === 'colaborador' && linkedEmpId && !chatTargetEmpId && (
        <button
          className="fab"
          onClick={() => setChatTargetEmpId(linkedEmpId)}
          title="Mensajes con el administrador"
        >
          💬
      ...
      {confirm && (
        ...
        >
          <div className="modal" style={{ maxWidth: 400 }}>
            ...
            >
              {confirm.type === 'emp'
                ? `¿Eliminar al colaborador "${confirm.item.name}"? Se perderán todas sus empresas y obligaciones.`
                : confirm.type === 'co'
                ? `¿Eliminar la empresa "${confirm.item.name}"? Se eliminarán todas sus obligaciones.`
                : '¿Eliminar esta obligación permanentemente?'}
            </p>
            <div
              style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}
            >
              <button className="btn bg" onClick={() => setConfirm(null)}>
                Cancelar
              </button>
              <button
                className="btn bd"
                ...
                onClick={async () => {
                  if (confirm.type === 'emp')
                    await saveEmps(
                      emps.filter((e) => e.id !== confirm.item.id)
                    );
                  else if (confirm.type === 'co')
                    await saveCos(
                      isAdmin ? selEmp.id : linkedEmpId,
                      (cos[isAdmin ? selEmp.id : linkedEmpId] || []).filter(
                        (c) => c.id !== confirm.item.id
                      )
                    );
                  else
                    await saveObs(
                      selCo.id,
                      (obs[selCo.id] || []).filter((o) => o.id !== confirm.item)
                    );
                  setConfirm(null);
                }}
              >
...
/* ══════════════════════════════════════════
   CHAT BOX
   ══════════════════════════════════════════ */
function ChatBox({ emp, messages, isAdmin, onClose, onSend }) {
    ...
    <div className="chatBox">
      <div className="chatHead">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, color: '#E2D9C8', fontSize: 14 }}>
              {isAdmin ? emp?.name || 'Colaborador' : 'Administrador'}
            </div>
            <div
              style={{
                fontSize: 10,
      ...
      <div className="chatBody" ref={bodyRef}>
        ...
        {messages.map((m) => {
          ...
          return (
            ...
            >
              <div className={`msg ${mine ? 'msgMe' : 'msgOther'}`}>
                {m.text}
              </div>
  ...
  return (
    <Shell title="➕ Nuevo Colaborador" onClose={onClose}>
      ...
      <div style={{ marginBottom: 26 }}>
        ...
        <input
          className="inp"
          placeholder="Ej. Contadora, Auxiliar Fiscal..."
          value={role}
      ...
      >
        💡 Después de crear el colaborador, pídele que inicie sesión con su
        Google. Aparecerá en "Pendientes" para que lo vincules a este registro.
      </div>
    ...
    <Shell title="🏢 Nueva Empresa" onClose={onClose}>
      ...
      <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
        ...
        <button
          className="btn bp"
          disabled={!name.trim()}
          onClick={() => onSave({ id: uid(), name: name.trim(), type })}
        >
          Agregar
        </button>
      </div>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                <button
                  ...
                  onClick={() => setMode({ ...mode, [p.uid]: 'link' })}
                >
                ...
                <button
                  ...
                  onClick={() => setMode({ ...mode, [p.uid]: 'new' })}
                >
                  Crear nuevo
                </button>
              </div>
              {m === 'link' ? (
                <select
                  className="sel"
                  style={{ padding: '8px 12px', fontSize: 13, marginBottom: 8 }}
                  value={linkTo[p.uid] || ''}
                  onChange={(e) =>
                    setLinkTo({ ...linkTo, [p.uid]: e.target.value })
                  }
                >
                ...
                >
                  <input
                    ...
                    onChange={(e) =>
                      setNewData({
                        ...newData,
                        [p.uid]: { ...newData[p.uid], name: e.target.value },
                      })
                    }
                  />
                  <input
                    ...
                    onChange={(e) =>
                      setNewData({
                        ...newData,
                        [p.uid]: { ...newData[p.uid], role: e.target.value },
                      })
                    }
                  />
                </div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  ...
                  onClick={() => {
                    if (m === 'link') {
                      if (linkTo[p.uid])
                        onAuthorize(p, linkTo[p.uid], false, null);
                    } else {
                      const data = newData[p.uid] || {
                        name: p.name || p.email,
                      };
